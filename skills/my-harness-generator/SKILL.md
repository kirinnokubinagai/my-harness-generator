...
```
${CLAUDE_PLUGIN_ROOT:-/my-harness-generator}/                 ジェネレータ本体（git 管理推奨）
...
```bash
bash ${CLAUDE_PLUGIN_ROOT:-/my-harness-generator}/scripts/install-skills-globally.sh
```
ジェネレータを更新（`git pull`）したあとに実行すると、~/.claude/skills/ が最新に同期される。

### 3. 既存プロジェクトに対して bootstrap を回す
```bash
bash ${CLAUDE_PLUGIN_ROOT:-/my-harness-generator}/scripts/bootstrap.sh /path/to/project          # 対話
bash ${CLAUDE_PLUGIN_ROOT:-/my-harness-generator}/scripts/bootstrap.sh /path/to/project --config <file>  # 非対話
```
...
- ジェネレータを更新したい → `cd ${CLAUDE_PLUGIN_ROOT:-/my-harness-generator} && git pull && bash scripts/install-skills-globally.sh`
