...
**プロンプト記法の絶対ルール**（Codex `image_gen` = gpt-image-2 に渡すとき）:

`gpt-image-2` は **拡散モデル** であり、自然言語の視覚的描写から画像を生成する。座標・ピクセル・コード風の指示は品質を下げる。プロンプトには以下を **絶対に書かない**:

- ❌ 座標指定（`(x=10, y=20)`、`top: 30px`、`left: 50%` など）
- ❌ ピクセル単位のサイズ（`24px font`、`border 2px` など）— 解像度自体（1024×1024）の指定だけは OK
- ❌ SVG パス・CSS プロパティ・HTML タグ
- ❌ コード片（`<div>`、`#color: ...`、`fillStyle = ...`）
- ❌ レイアウト・グリッド指示の数値化（`grid-cols-3`、`flex-row gap-4` など）

代わりに **絵を口頭で説明する形** で書く:

- ✅ 「中央に丸いマーク、その下に小さなテキスト」
- ✅ 「ミニマル、ベクター調、フラットデザイン、太い線」
- ✅ 「ターコイズと白の 2 色配色、温かい雰囲気」
- ✅ 「シンプルな幾何学形状、クリーンな余白」
- ✅ 「左上にロゴ、メインエリアにカード一覧、下部にナビゲーションバー」

UI モックでも同じ。「ヘッダーに検索バー、下にカード形式の一覧 6 枚、右上にハンバーガーメニュー」のような **構造の口語的記述** で書く。`tailwindcss` のクラス名や React コンポーネント名を書かない。

プロンプト全体の長さは **概ね 100〜200 単語** に抑える。長すぎると指示が薄まる。

**固定質問**（1 ターン 1 問）:

1. **主色は？**（例: `#14b8a6`、または「青系」「緑系」のような曖昧表現も可、Claude が #コード化）
2. **ロゴの方向性は？**（選択肢: マーク / ワードマーク / コンビ）
3. **印象は？**（選択肢: シンプル / プロフェッショナル / カジュアル / ポップ）— **3 形容詞や「世界観」は聞かない**
4. **主要画面を 3〜5 個リストアップしてください**（例: ログイン / ホーム / 詳細 / 設定）
