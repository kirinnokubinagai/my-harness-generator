...
2. **「誰が使いますか？」**（例: 自分専用 / 社内チーム数人 / 一般公開）
3. **「MVP に必要な機能を 3〜7 個リストアップしてください」**（番号付き）

これだけ。「既存サービスではダメな理由」「5 年後の成功状態」「ペルソナ何種類」等は **聞かない**。
...
~/my-harness-generator/scripts/codex-ask.sh --role analyst \
  ...
  "プロジェクト概要: <一言>。ターゲット: <誰>。MVP 機能リスト: <列挙>。論理矛盾・曖昧さ・抜け漏れを指摘してください。"
```
