/**
 * 概要: 冪等性 middleware (`Idempotency-Key` ヘッダー対応)。
 *       POST / PUT / PATCH / DELETE のうち `Idempotency-Key` が付いたものは、
...
export function idempotency(): MiddlewareHandler<{ Bindings: IdempotencyEnv }> {
  return async (c, next) => {
