...
構造化インタビューであなたから引き出し、必要なら Codex に第二意見を求め、ロゴ・UI モックを生成し、技術スタックを決定し、ブランチ保護・CI・hooks・セキュリティ middleware・runbook・並列レーンの規約までセットされた**プロダクション級**モノレポを 1 つのスラッシュコマンドから生成。最初のコミットは緑、`main`/`stage`/`dev` は保護済み、会話はマスク済みで `dev/docs/talk/` に自動記録。
...
| 5 | Visual | ロゴ 3 案 + プラットフォーム別 UI モック 3〜5 個 (`gpt-image-2`)、各モックを深掘り |
