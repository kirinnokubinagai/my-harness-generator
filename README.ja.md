...
- 任意: Codex 連携を使うなら `npm install -g @openai/codex`。認証方式は `codex login`（ChatGPT サブスクリプション）または `export OPENAI_API_KEY=sk-...`（API キー従量課金）から選べます。Codex を有効化するとインタビューで選択を求められます。
...
CODEX_AUTH=subscription       # subscription（ChatGPT Plus/Pro/Team/Enterprise で `codex login`）または api-key（OPENAI_API_KEY 環境変数、トークン従量課金）
